...
<jsp:setProperty name="form" property="formURL" value="borrow.jsp" />
...
if (refURL!=null){
	if((refURL.indexOf("borrow.jsp") < 0) || (dbprocess.isProcessComplete())) {
	...
	if (formName.equals("resetForm")) {
		...
		if( !(borrowerID.equals("")) ) {
			...
			if(borrowerID.equals(staff.getUserID())) {
				...
				query = "SELECT * FROM libuser WHERE UserID ='" +borrowerID +"'";
				try {  
    			...
    			if (myResultSet != null){
					try{
						...
						System.err.println("borrow.jsp Error connecting: " +sqle);		
					}
				}
				if(borrowerID.equals("")) {
		...
		if( !(itemCallNumber.equals("")) ) {
    		...
    		if (myResultSet != null){
				try{
					...
					System.err.println("catch borrow.jsp Error connecting: " +sqle);		
				}
			}
			if(itemCallNumber.equals(""))
...
%>
			<jsp:forward page="/formprocess"></jsp:forward>
                ...
                <tr> 
                  <td width="17%"> 
                    <% if(borrowerID.equals("")){
						...
						session.putValue("bb",borrowerID);
...
<form name="form5" method="post" action="borrow1.jsp">

               
<input type="hidden" name="itemcallnumber" size="22" maxlength="20"
