...
<jsp:setProperty name="form" property="formURL" value="penalty_list.jsp" />
<%  response.setHeader("Cache-Control", "no-cache");
	response.setHeader("Pragma", "no-cahce");
// 	URLcurrent = "penalty_list.jsp";
...
<%
	...
	if((queryOrderField.equals("")) || (queryOrderType.equals("")))
		queryOrder = " ORDER BY borrowerID ";
	else 
...
%>
      <form name="form1" action="penalty_list.jsp?<%= queryParameter%>" method="post">
        <center>
...
%>
      <form name="form2" method="post" action="penalty_list.jsp?<%= queryParameter%>">

          <input type="hidden" name="recDisplayNum" maxlength="2" size="4" value="<%= recDisplayNum %>">

        </form>
<%
	queryParameter="&borrowerID=" +borrowerID +"&recDisplayNum=" +recDisplayNum;
%>
        <form name="form2" method="post" action="penalty_list.jsp">


          
        <table width="100%" border="0" cellspacing="1" cellpadding="1" align="center">
          <tr> 
            <th width="18%" bgcolor="#D3DCE3">Borrower ID<a href=penalty_list.jsp?&queryOrderField=borrowerID&queryOrderType=ASC&<%= queryParameter%>><br>
              <% if(queryOrderField.equals("borrowerID") && queryOrderType.equals("ASC")) 
				...
				%>
              </a> <a href=penalty_list.jsp?&queryOrderField=borrowerID&queryOrderType=DESC&<%= queryParameter%>> 
            ...
            <th width="20%" bgcolor="#D3DCE3">Borrow Record Number*<a href=penalty_list.jsp?&queryOrderField=borrowRecNumber&queryOrderType=ASC&<%= queryParameter%>> 
              <br>
              <% if(queryOrderField.equals("borrowRecNumber") && queryOrderType.equals("ASC")) 
					...
					out.println("<img src='icon_sort_up_off.gif' width='20' height='7' border='0'>");
				%>
              </a> <a href=penalty_list.jsp?&queryOrderField=borrowRecNumber&queryOrderType=DESC&<%= queryParameter%>> 
            ...
            <th width="18%" bgcolor="#D3DCE3">Overdue Day<a href=penalty_list.jsp?&queryOrderField=overdueDay&queryOrderType=ASC&<%= queryParameter%>><br>
              <% if(queryOrderField.equals("overdueDay") && queryOrderType.equals("ASC")) 
					out.println("<img src='icon_sort_up_on.gif' width='20' height='7' border='0'>");
					else
					out.println("<img src='icon_sort_up_off.gif' width='20' height='7' border='0'>");
				%>
              </a> <a href=penalty_list.jsp?&queryOrderField=overdueDay&queryOrderType=DESC&<%= queryParameter%>> 
            ...
            <th width="15%" bgcolor="#D3DCE3">Penalty Amount ($)<a href=penalty_list.jsp?&queryOrderField=penaltyAmount&queryOrderType=ASC&<%= queryParameter%>><br>
              <% if(queryOrderField.equals("penaltyAmount") && queryOrderType.equals("ASC")) 
					out.println("<img src='icon_sort_up_on.gif' width='20' height='7' border='0'>");
					else
					out.println("<img src='icon_sort_up_off.gif' width='20' height='7' border='0'>");
				%>
              </a> <a href=penalty_list.jsp?&queryOrderField=penaltyAmount&queryOrderType=DESC&<%= queryParameter%>> 
	...
	// Get the request record result from search query 
	 query = "SELECT BORROWRECNUMBER,ITEMCALLNUMBER,STAFFIDBORROW,STAFFIDRETURN,BORROWERID,BORROWEREMAIL,BORROWDATE,DUEDATE,RETURNDATE,BORROWSTATUS,(sysdate-to_date(DUEDATE)) AS OVERDUEDAY FROM borrowrec "+ queryCondition + queryOrder; 
    	...
    	if (myResultSet != null){
			...
			try{
				while(myResultSet.next()) {
					String bRecNo=myResultSet.getString("borrowRecNumber");
	    			if (rowCount % 2 == 0)
					...
					out.println("</TD><TD><a href='borrowrec_view.jsp?borrowRecNumber=" +bRecNo+"'>" +bRecNo+"</a>");
				...
				System.err.println("penality_list.jsp Error connecting: " +sqle);		
			}
		} 
%>
        </table>
