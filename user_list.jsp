...
<jsp:setProperty name="form" property="formURL" value="user_list.jsp" />
<%  response.setHeader("Cache-Control", "no-cache");
	response.setHeader("Pragma", "no-cahce");
// 	URLcurrent = "user_list.jsp";
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr bgcolor="#f1f1f1"> 
    ...
    <td valign="top" width="100%" bgcolor="#f1f1f1"> 
      ...
      <center>
        [<a href="user_add.jsp">Add new user recod</a>] 
...
<%
	...
	String queryCount = "SELECT count(*) FROM libuser" + queryCondition;
   	try {  
	...
	query = "SELECT * FROM libuser" + queryCondition;
...
<%
	// Display the search result 
   	if (myResultSet != null){
		System.out.println("user_list.jsp helloworld 0");
			int rowCount = 1 ; // set counter to separate even or odd
			try{
				while(myResultSet.next()) {
					int userrecno=myResultSet.getInt("userRecNumber");
					System.out.println("user_list.jsp helloworld");
	    			if (rowCount % 2 == 0){
						System.out.println("user_list.jsp helloworld1");
	       				out.println ("<tr bgcolor=#CCCCCC align=left><td>");
					}else {
					System.out.println("user_list.jsp helloworld2");
					out.println ("<tr bgcolor=#DDDDDD align=left><td>");
					}
					if((login.getUserType().equals("admin"))){
						System.out.println("user_list.jsp helloworld3");
						out.println("<input type=\"checkbox\" name=\"del_rec\" value=\"" +userrecno +"\">");
					}else{
						System.out.println("user_list.jsp helloworld4");
						//out.println("</TD>&nbsp<TD>");
					}
					System.out.println("user_list.jsp helloworld5");
					out.println("</TD><TD><a href='user_edit.jsp?userRecNumber=" +userrecno +"'>" +myResultSet.getString("userID") +"</a>");
//					out.println("</TD><TD><a href='user_edit.jsp?userRecNumber=" +myResultSet.getString("userID") +"'>record</a>");
					System.out.println("user_list.jsp helloworld6");
					
					out.println("</TD><TD>" +myResultSet.getString("userFirstName"));
					System.out.println("user_list.jsp helloworld7");
					
					out.println("</TD><TD>" +myResultSet.getString("userLastName"));
					System.out.println("user_list.jsp helloworld8");
					
					out.println("</TD><TD>" +myResultSet.getString("userType"));
					System.out.println("user_list.jsp helloworld9");
					
					out.println("</TD></TR>");
					rowCount = rowCount+1;					
				}
			} catch(SQLException sqle) {
				System.err.println(" user_list.jsp Error connecting: " +sqle);		
			}
		} 
%>
      </table>
<% if((login.getUserType().equals("admin"))){ 
%>
        <p>&nbsp; <img src="arrow_ltr.gif" width="38" height="22">With 
          selected: 
