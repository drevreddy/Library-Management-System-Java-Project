...
function validateComplete(formObj) {
	if(emptyField(formObj.userRecNo)) {
		alert("Please fill out the userRecNo"); formObj.userRecNo.focus(); 
	}else if(emptyField(formObj.userID)) {
...
<%
		...
		String errorMsg;
		String queryCount = "SELECT MAX(USERRECNUMBER) as userRecNumber FROM LIBUSER";
    	try {  
		...
		while (rsCount.next()){
			userRecNumber=rsCount.getInt(1) + 1;
		}%>

<form name="user_form" method="post" action="./formprocess">
  * Compulsory to fill out<br><br>
  <table width="100%" border="0" cellspacing="1" cellpadding="1">
    <tr> 
      <td width="31%">User Record Number</td>
      <td width="69%"> 
        <p> 
          <input type="text" name="userRecNo" maxlength="10" size="12" value=<%=userRecNumber%>>
        </p>
      </td>
    </tr>
	<tr> 
...
%>
  ...
  <p> 
	...
	if( (loginUserType.equals("admin")) || debug ){
		...
		out.println(submitStm + resetStm);
	} else if(form.getFormURL().equals("myaccount.jsp")) {
		out.println(submitStm + resetStm);
