...
<jsp:setProperty name="form" property="formURL" value="libraryitem_list.jsp" />
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr bgcolor="#f1f1f1"> 
    ...
    <td valign="top" width="100%" bgcolor="#f1f1f1"> 
      ...
      <center>
        [<a href="libraryitem_form.jsp">Add new item recod</a>] 
...
<form name='userList' action='./formprocess'>
	...
	query = "SELECT * FROM libraryitem" + queryCondition + queryOrder;
  		try {  
      		Connection connection = connectionPool.getConnection();
    	...
    	if (myResultSet != null){
			...
			try{
				while(myResultSet.next()) {
					int iRecNo=myResultSet.getInt("itemRecNumber");
	    			if (rowCount % 2 == 0)
	       				out.println ("<tr bgcolor=#CCCCCC align=left><td>");
					else 
					out.println ("<tr bgcolor=#DDDDDD align=left><td>");
					if((login.getUserType().equals("admin"))){
						out.println("<input type=\"checkbox\" name=\"del_rec\" value=\"" +iRecNo+"\">");
					}else{
						//out.println("</TD>&nbsp<TD>");
					}
					
					out.println("</TD><TD><a href='libraryitem_edit.jsp?itemRecNumber=" +iRecNo+"'>" +myResultSet.getString("itemTitle") +"</a>");
				...
				System.err.println("libraryitem_list.jsp Error connecting: " +sqle);		
			}
		} 
%>
        </table>
